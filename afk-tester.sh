...
# Jira-gated AFK loop for Ralph (Tester Mode)
# Checks into Jira for "needs-tests" tasks.
...
while true; do
  ...
  if [[ "$task_count" -eq 0 ]]; then
    echo "[$(date '+%H:%M:%S')] No Tester tasks assigned. Sleeping ${POLL_INTERVAL}s..."
    sleep "$POLL_INTERVAL" &
    child_pid=$!
  ...
  echo "------- TESTER ITERATION $iteration ($task_count tasks) --------"
  ...
  if [[ "$result" == *"<promise>ABORT</promise>"* ]]; then
    echo "Ralph (Tester) aborted at iteration $iteration."
    exit 1
  fi
