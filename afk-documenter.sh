...
# Jira-gated AFK loop for Ralph (Documenter Mode)
# Checks into Jira for "Done" tasks not yet documented.
...
while true; do
  ...
  if [[ "$task_count" -eq 0 ]]; then
    echo "[$(date '+%H:%M:%S')] No Documenter tasks assigned. Sleeping ${POLL_INTERVAL}s..."
    sleep "$POLL_INTERVAL" &
    child_pid=$!
  ...
  echo "------- DOCUMENTER ITERATION $iteration ($task_count tasks) --------"
  ...
  if [[ "$result" == *"<promise>ABORT</promise>"* ]]; then
    echo "Ralph (Documenter) aborted at iteration $iteration."
    exit 1
  fi
