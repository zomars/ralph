...
# Jira-gated AFK loop for Ralph (Planner Mode)
# Checks into Jira for tasks with empty descriptions.
...
while true; do
  ...
  if [[ "$task_count" -eq 0 ]]; then
    echo "[$(date '+%H:%M:%S')] No Planner tasks assigned. Sleeping ${POLL_INTERVAL}s..."
    sleep "$POLL_INTERVAL" &
    child_pid=$!
  ...
  echo "------- PLANNER ITERATION $iteration ($task_count tasks) --------"
  ...
  if [[ "$result" == *"<promise>ABORT</promise>"* ]]; then
    echo "Ralph (Planner) aborted at iteration $iteration."
    exit 1
  fi
