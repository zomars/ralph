...
# Jira-gated AFK loop for Ralph (Refactorer Mode)
# Checks into Jira for "tech-debt" tasks.
...
while true; do
  ...
  if [[ "$task_count" -eq 0 ]]; then
    echo "[$(date '+%H:%M:%S')] No Refactor tasks assigned. Sleeping ${POLL_INTERVAL}s..."
    sleep "$POLL_INTERVAL" &
    child_pid=$!
  ...
  echo "------- REFACTORER ITERATION $iteration ($task_count tasks) --------"
  ...
  if [[ "$result" == *"<promise>ABORT</promise>"* ]]; then
    echo "Ralph (Refactorer) aborted at iteration $iteration."
    exit 1
  fi
