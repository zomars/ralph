...
import { checkApproval } from "./pr-approval.mjs";
...
// Approval gate for a PR. Delegates the rule to lib/pr-approval.mjs
// (single source of truth shared with the shell-side queue filter).
//
// gh CLI's --json doesn't surface author.__typename, so we fetch via GraphQL
// to preserve App detection.
async function ghCheckApproval(prNumber) {
  const repo = await ghJson("repo", "view", "--json", "owner,name");
    ...
    `query={ repository(owner: "${repo.owner.login}", name: "${repo.name}") {
        pullRequest(number: ${prNumber}) {
          ...
          latestReviews(first: 20) {
            nodes { state authorAssociation author { __typename } }
          }
        }
      } }`
  );
  const pr = data?.data?.repository?.pullRequest;
  if (!pr) return { approved: false, reason: `PR #${prNumber} not found` };
  return checkApproval(pr);
}
    ...
    async ({ prNumber }) => {
      try {
        const pr = await ghJson(
          ...
          String(prNumber),
          "--json",
          "mergeable,statusCheckRollup,headRefName,baseRefName,isDraft"
        );

        const approvalCheck = await ghCheckApproval(prNumber);
        const unresolvedCount = await ghUnresolvedThreadCount(prNumber);

        const mergeable = pr.mergeable === "MERGEABLE";
        const approved = approvalCheck.approved;
        ...
        if (!approved) blockers.push(approvalCheck.reason);
        if (!threadsResolved) {
    ...
    async ({ prNumber }) => {
      try {
        ...
        if (!approvalCheck.approved)
          throw new Error(`Cannot merge: ${approvalCheck.reason}`);
        const unresolvedCount = await ghUnresolvedThreadCount(prNumber);
