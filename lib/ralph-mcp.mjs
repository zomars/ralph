    ...
    async ({ prNumber }) => {
      try {
        const pr = await ghJson(
          ...
          String(prNumber),
          "--json",
          "mergeable,reviewDecision,reviewThreads,statusCheckRollup,headRefName,baseRefName,isDraft"
        );

        const mergeable = pr.mergeable === "MERGEABLE";
        const approved = pr.reviewDecision === "APPROVED";
        const threadsResolved = (pr.reviewThreads || []).every(
          (t) => t.isResolved
        );
        ...
        if (!threadsResolved) {
          ...
          blockers.push(`${unresolved} unresolved review thread(s)`);
        }
        if (!ciGreen) {
  ...
  server.tool(
    ...
    async ({ prNumber }) => {
      try {
        ...
        const unresolvedThreads = (pr.reviewThreads || []).filter(
          (t) => !t.isResolved
        );
        if (unresolvedThreads.length > 0)
          throw new Error(
            `Cannot merge: ${unresolvedThreads.length} unresolved thread(s)`
          );
